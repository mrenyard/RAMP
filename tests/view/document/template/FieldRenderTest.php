...
class FieldRenderTest extends TestBase
{
  ...
  public function testFieldColorRender()
  {
    ...
    $this->testObject->validate(PostData::build(array(
      'comprehensive-record:1|1|1:primary-color' => '#GGGGGG'
    )));
    ob_start();
    $view->render();
    $output2 = ob_get_clean();
    $this->assertSame(
      '          <div class="color input field compact error" title="Primary colour, main identifiable brand colour, the core colour, commonly incorporated into a companies logo.">' . PHP_EOL .
      '            <label for="comprehensive-record:1|1|1:primary-color">Primary Colour</label>' . PHP_EOL .
      '            <input id="comprehensive-record:1|1|1:primary-color" name="comprehensive-record:1|1|1:primary-color" type="color" tabindex="1" value="" />' . PHP_EOL .
      '            <span class="hint">representing the luminescent gradiant of red, green and blue, a hash followed by three pairs of hexadecimal characters (0 through 9 to F) formated: (#000000) with a length of exactly 7. <em>Previous value was: #20771E.</em></span>' . PHP_EOL .
      '          </div>' . PHP_EOL . '',
      $output2
    );
  }
  ...
  public function testFieldTelRender()
  {
    ...
    // Defaults as set on model (ComprehensiveRecord). 
    $this->assertSame('input field', (string)$view->type);
    $this->assertSame('input field', (string)$view->class);
    $this->assertSame('tel', (string)$view->inputType);
    $this->assertSame(ComprehensiveRecord::$mobileTitle, $view->title);
    $this->assertSame(ComprehensiveRecord::$mobilePlaceholder, $view->placeholder);
    $this->assertSame(' placeholder="' . ComprehensiveRecord::$mobilePlaceholder . '"', (string)$view->attribute('placeholder'));
    $this->assertSame('Mobile', (string)$view->label);
    ob_start();
    $view->render();
    $output = ob_get_clean();
    $this->assertSame(
      '          <div class="tel input field required" title="The number used to contact (call or text) said particular persons mobile device.">' . PHP_EOL .
      '            <label for="comprehensive-record:1|1|1:mobile">Mobile</label>' . PHP_EOL .
      '            <input id="comprehensive-record:1|1|1:mobile" name="comprehensive-record:1|1|1:mobile" type="tel" tabindex="0" placeholder="e.g. 07744 654321" required="required" pattern="(?:\+[1-9]{1,3} ?\(0\)|\+[1-9]{1,3} ?|0)[0-9\- ]{8,12}" maxlength="20" value="07744 123123" />' . PHP_EOL .
      '            <span class="hint">valid telephone number string with a maximum charactor length of 20</span>' . PHP_EOL .
      '          </div>' . PHP_EOL . '',
      $output
    );
    // Overrridden within view.
    $view->style = Str::set('compact');
