...
class RecordComponentTest extends \PHPUnit\Framework\TestCase
{
  ...
  /**
   * Collection of assertions for \ramp\model\business\RecordComponent::id.
  ...
  public function testOffsetGet()
  {
    try {
      ...
      $this->assertInstanceOf('\ramp\model\business\RecordComponent', $this->testObject[1]);
      $this->assertSame($this->testChild2, $this->testObject[1]);
      $this->assertInstanceOf('\ramp\model\business\RecordComponent', $this->testObject[2]);
  ...
  public function testOffsetSetOffsetUnset()
  {
    try {
      $this->testObject[3] = new Option(3, Str::set('No Option'));
        ...
        $this->assertSame('ramp\model\business\field\Option NOT instanceof tests\ramp\model\business\mocks\RecordComponentTest\MockRecordComponent', $expected->getMessage());

        $object = new MockRecordComponent('Forth child');
  ...
  public function testGetErrors()
  {
    ...
    // Returns same results on subsequent call, while RecordComponents are in same state.
    ...
    // Calls on sub RecordComponents return expected sub set of Errors.
    $child2Errors = $this->testChild2->errors;
    $this->assertSame('Second child error message', (string)$errors[0]);
    $this->assertSame('First grandchild error message', (string)$errors[1]);

    // Calls on sub RecordComponents return expected sub set of Errors, even on grandchildren.
